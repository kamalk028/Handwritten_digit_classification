...
def preprocess():
    ...
    # Your code here.
    count = 0
    compressedData = np.sum(train_data, axis = 0)
    ...
    train_wo_zero = np.zeros(shape=(1, 50000))
    validation_wo_zero = np.zeros(shape=(1, 10000))
    test_wo_zero = np.zeros(shape=(1, 10000))

    
    for i in range (compressedData.shape[0]):
        if(compressedData[i] != 0):
           train_row = [col[i] for col in train_data]
           validation_row = [col[i] for col in validation_data]
           test_row = [col[i] for col in test_data]
           train_wo_zero = np.vstack((train_row, train_wo_zero))
           validation_wo_zero = np.vstack((validation_row, validation_wo_zero))
           test_wo_zero = np.vstack((test_row, test_wo_zero))
           
    train_wo_zero = train_wo_zero.transpose()
    validation_wo_zero = validation_wo_zero.transpose()
    test_wo_zero = test_wo_zero.transpose()
    
    train_data = train_wo_zero
    validation_data = validation_wo_zero
    test_data = test_wo_zero
...
file = open("nnscript_out.txt", "a+")
file.write("Writing outputs.. ")
file.write("\n")
...
while n_hidden <= 60:
    ...
    while lambdaval <=60:
        test_cnt+=1
        file = open("nnscript_out.txt", "a+")
        file.write("Test Count: "+str(test_cnt))
        file.write("\n")
        file.write("Hidden units: "+str(n_hidden) )
        file.write("\t")
        file.write("Lambda value: "+str(lambdaval))
        file.write("\n")
        file.close()
        print(lambdaval)
        ...
        import pickle
        pickle.dump(obj, open('params_'+str(n_hidden)+'_'+str(lambdaval)+'.pickle', 'wb'))
        ...
        etime = t2 - t1
        sec = divmod(etime.total_seconds(), 60)
        time_taken = "Time Taken in minutes : "+str(sec)
        print(time_taken)
        
        file = open("nnscript_out.txt", "a+")
        file.write("\n")
        file.write(training_set_acc)
        file.write("\n")
        file.write("\n")
        file.write(validation_set_acc)
        file.write("\n")
        file.write("\n")
        file.write(test_set_acc)
        file.write("\n")
        file.write("\n")
